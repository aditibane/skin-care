...
const QuestionFourteen = () => {
	...
	const [options, setOptions] = useState([
		...
		{ id: 4, value: "Polluted", selected: false },
	]);
	...
	const handleClick = () => {
		...
		fetch(ENDPOINT, {
			...
			.then(function (data) {
				setMessage(data.predicted_label ? data.predicted_label : data.message);
			...
			<div className="h-[100vh] w-[70vw] flex justify-center items-center">
				<div className="h-[50vh] w-[50vw]">
					...
					<div className="flex w-[50vw] justify-center mt-[2rem]">
						<button
							onClick={handleClick}
							className="w-[30rem] hover:bg-[#6fb09a] transition text-[20px] font-[500] opacity-[0.9] h-[3rem] bg-[#7BC4AB] text-[#fff] transition border rounded border-[#7BC4AB]"
						>
							Click here to get your personalised product
						</button>
					</div>
